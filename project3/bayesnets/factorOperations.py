...
def joinFactors(factors):
    ...
    for unconditionedVar in unconditioned:
        if unconditionedVar in conditioned:
            conditioned.remove(unconditionedVar)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        assignments = newFactor.getAllPossibleAssignmentDicts()
        eliminateVars = factor.variableDomainsDict()[eliminationVariable]
        for assignment in assignments:
            prob = 0
            for eliminationVar in eliminateVars:
                assignment[eliminationVariable] = eliminationVar
                prob += factor.getProbability(assignment) 
            newFactor.setProbability(assignment, prob)
        return newFactor
