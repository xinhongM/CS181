...
def sentence3() -> Expr:
    ...
    alive1 = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))
    alive2 = ~(PacmanAlive_0 & PacmanBorn_0)
    born = PacmanBorn_0
    return conjoin(alive1 ,alive2, born)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    S = findModel(~(premise >> conclusion))
    return not S
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    result = []
    S = itertools.combinations(literals, 2)
    for i in S:
        dis = disjoin(~i[0],~i[1])
        result.append(dis)
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    A = atLeastOne(literals)
    B = atMostOne(literals)
    return conjoin(A, B)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #return PropSymbolExpr(pacman_str, x, y, now) % atLeastOne(possible_causes)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    #pacphysics_sentences.append(conjoin(all_c))
    #
    atLeastSquare = []
    for square in non_outer_wall_coords:
        x, y = square
        atLeastSquare.append(PropSymbolExpr(pacman_str, x, y, time = t) )
    pacphysics_sentences.append(exactlyOne(atLeastSquare))
    #
    oneAction = [PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), PropSymbolExpr('East', time=t), PropSymbolExpr('West', time=t)]
    pacphysics_sentences.append(exactlyOne(oneAction))
    # #
    if sensorModel  != None and sensorModel != checkLocationSatisfiability:
    # if sensorModel  != None:
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    return (findModel(conjoin(KB_con, PropSymbolExpr(pacman_str, x1,y1,time=1))), findModel(conjoin(KB_con, ~PropSymbolExpr(pacman_str, x1,y1,time=1))) )
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            oneLocation.append(PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(oneLocation))
        #
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        goal_food = []
        for x,y in food:
            goal_food.append(PropSymbolExpr(food_str, x, y, time = t))
        goal = ~disjoin(goal_food) # no one food
        # determin
        model = findModel(conjoin(goal, conjoin(KB)))
        if model:
            return extractActionSequence(model, actions)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            pac_ATxy_t = PropSymbolExpr(pacman_str, x, y, time = t)
            # KB >> pac at x,y at t
            if entails(conjoin(KB), pac_ATxy_t):
                KB.append(pac_ATxy_t)
            # KB >> pac not at x,y at t
            if entails(conjoin(KB), ~pac_ATxy_t):
                KB.append(~pac_ATxy_t)
            if findModel(conjoin(pac_ATxy_t, conjoin(KB))):
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x,y in non_outer_wall_coords:
            pac_ATxy_t = PropSymbolExpr(wall_str, x, y)
            # KB >> wall at x,y
            if entails(conjoin(KB), pac_ATxy_t):
                known_map[x][y] = 1
                KB.append(pac_ATxy_t)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            ######## part1 location ##########
            pac_ATxy_t_wall = PropSymbolExpr(wall_str, x, y)
            # KB >> wall at x,y
            if entails(conjoin(KB), pac_ATxy_t_wall):
                known_map[x][y] = 1
                KB.append(pac_ATxy_t_wall)
            # KB >> wall not at x,y
            if entails(conjoin(KB), ~pac_ATxy_t_wall):
                known_map[x][y] = 0
                KB.append(~pac_ATxy_t_wall)
        ...
        for x,y in non_outer_wall_coords:
            ########### part2 map ##########
            pac_ATxy_t_pac = PropSymbolExpr(pacman_str, x, y, time = t)
            # KB >> pac at x,y at t
            if entails(conjoin(KB), pac_ATxy_t_pac):
                KB.append(pac_ATxy_t_pac)
            # KB >> pac not at x,y at t
            if entails(conjoin(KB), ~pac_ATxy_t_pac):
                KB.append(~pac_ATxy_t_pac)
            
            if findModel(conjoin(pac_ATxy_t_pac, conjoin(KB))):
                possible_locations.append((x,y))
