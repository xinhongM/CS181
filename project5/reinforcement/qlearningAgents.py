...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        originVal = self.getQValue(state, action)
        newQ = self.getValue(nextState)
        ans = (1 - self.alpha) * originVal + self.alpha * (self.discount * newQ + reward)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in featureVector:
          result += w[feature] * featureVector[feature]
        return result
    ...
    def update(self, state, action, nextState, reward):
        ...
        newQ = self.discount * self.computeValueFromQValues(nextState) + reward
        diff = newQ - Q
