...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                if bestAction != None:
                    Q = self.computeQValueFromValues(state, bestAction)
                    newValue[state] = Q
                else:
                    newValue[state] = self.values[state]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for newState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            newValue = self.getValue(newState)
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                ...
                actions  = self.mdp.getPossibleActions(state)
                bestAction = self.computeActionFromValues(state)
                Qmax = self.computeQValueFromValues(state, bestAction)
            ...
            if not queue.isEmpty():
                ...
                for predecessor in predecessors[state]:
                    if not self.mdp.isTerminal(predecessor):
                        ...
                        Qvalue = self.computeQValueFromValues(predecessor, bestAction)
